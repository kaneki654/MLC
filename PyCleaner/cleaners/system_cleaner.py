...
class SystemOptimizer:
    ...
    def set_cpu_governor(self, mode="performance"):
        """Sets CPU governor using sudo."""
        # Try cpupower first, then direct sysfs write loop
        cmd = f"cpupower frequency-set -g {mode} || for file in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do echo {mode} | tee $file; done"
        return self._run_privileged(cmd)

class StartupManager:
    def list_autostart(self):
        """Lists XDG autostart entries (enabled and disabled)."""
        apps = []
        ...
        for f in os.listdir(path):
            ...
            if f.endswith(".desktop"):
                apps.append({"name": f.replace(".desktop", ""), "path": full_path, "enabled": True})
            elif f.endswith(".desktop.bak"):
                apps.append({"name": f.replace(".desktop.bak", ""), "path": full_path, "enabled": False})
        return apps

    def toggle_app(self, path, enable=True):
        try:
            if enable:
                if path.endswith(".desktop.bak"):
                    new_path = path[:-4] # remove .bak
